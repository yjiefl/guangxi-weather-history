...
1. 在数据管理部分提供下载数据功能，要求下载的数据是完整的数据，包括城市、天气和所有其他数据信息，报错：2026-01-27 11:32:30,080 - werkzeug - INFO - 127.0.0.1 - - [27/Jan/2026 11:32:30] "POST /undefined/data/export-bulk HTTP/1.1" 405 -

### 后续改进

1. 考虑程序部署到服务器上或者云端，让其他人可以快速的使用
2. 运行start.command后，自动打开浏览器（✅已实现，仅限macOS）
3. 连接、断开后，前端有提示，按钮要变色提醒（✅已实现状态灯同步及按钮禁用逻辑）
4. 城市仅需要广西的城市，崇左、贵港的需要精确到县城
5. 在每个图上提示是哪个城市的数据